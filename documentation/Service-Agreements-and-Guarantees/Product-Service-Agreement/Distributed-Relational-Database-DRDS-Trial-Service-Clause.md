...
**本试用服务条款是京东云网站所有者（本协议中称为“京东云”网址：**[www.jdcloud.com](https://www.jdcloud.com/) **）与您就分布式关系数据库DRDS相关事项所订立的有效合约。您通过盖章、网络页面点击确认或以其他方式选择接受本试用服务条款，包括但不限于未点击确认本试用服务条款而事实上使用了京东云分布式关系数据库DRDS试用服务，即表示您与京东云已达成协议并同意接受本试用服务条款的全部约定内容。如若双方盖章文本与网络页面点击确认或以其他方式选择接受之服务条款文本，存有不一致之处，以双方盖章文本为准。**
...
1.1本条款中“服务”指：京东云向您提供[www.jdcloud.com](https://www.jdcloud.com/)网站上所展示的分布式关系数据库DRDS试用服务以及相关的技术及网络支持服务。

1.2 京东云提供的服务必须符合本试用服务条款的约定。

#### **2. 服务费用**

2.1 公测期间，用户可免费试用DRDS。
...
3.2.3 若服务涉及分布式关系数据库DRDS的，京东云仅负责数据库系统以下的底层部分及京东云提供的软件的运营维护，即分布式关系数据库DRDS的相关技术架构及操作系统等。数据库之上的应用部分由您自行负责。
...
7.1 服务期限自您分布式关系数据库DRDS创建成功之日起（而非自您获取分布式关系数据库DRDS的密码之日起）计算，并以您所缴纳的款项数额为依据确认服务期限。

7.2 发生下列情形，服务期限提前终止：

7.2.1 双方协商一致提前终止的；

7.2.2 您理解并充分认可，虽然京东云已经建立（并将根据技术的发展不断完善）必要的技术措施来防御包括计算机病毒、网络入侵和攻击破坏（包括但不限于DDOS）等危害网络安全事项或行为（以下统称该等行为），但鉴于网络安全技术的局限性、相对性以及该等行为的不可预见性，因此如因您网站遭遇该等行为而给京东云或者京东云的其他的网络或服务器（包括但不限于本地及外地和国际的网络、服务器等）带来危害，或影响京东云与国际互联网或者京东云与特定网络、服务器及京东云内部的通畅联系，京东云可决定暂停或终止服务。如果终止服务的，将按照实际提供服务时间计算服务费用，并且您仍应支付您尚未支付的服务费用（如有）。

7.2.3 京东云可提前在[www.jdcloud.com](https://www.jdcloud.com/) 上通告或给您发网站内通知或书面通知的方式终止本服务条款。届时京东云应将您已支付但未消费的款项退还至您的京东云账户。
