...
2.2.3.1.1全局样式主要包括字体、边距、以及动画效果设置。
 
2.2.3.1.2X/Y轴主要包括：网格、轴线、刻度值以及坐标字体样式设置
 
2.2.3.1.3图例主要包括：图例的位置、字体样式以及图表的设置。
 
2.2.3.1.4数据系列包括，数据系列的名称、折线的粗细、圆点的半径、显示的区域颜色以及是否显示标签。单击添加图标，可以新增新的数据系列
...
2.2.4.1保存：保存您对大屏做的设置。
2.2.4.2预览：您可以全屏查看您的维护好的大屏。在预览情况下，您可以检查您所绑定的数据。如果数据源是实时数据还将看到实时数据展示效果。
预览大屏如下图：
2.2.4.3发布大屏
...
⦁3.1	创建数据源连接
 进入数据源频道后点击左侧数据源页面左上角的“添加”
 在弹出的对话框中选择您要添加的数据源类型。目前支持两种RDS与数据仓库

添加成功后, 数据库将出现在数据源列表中, 此时您可以使用该库下的所有数据表
3.2编辑数据源：
在数据源列表中，点击删除和编辑数据源。
  
