...
- **多地域多可用区：**
覆盖国内华北一区（已开放B区，正在筹备开放A区）、华南一区（筹备中）、华东一区（筹备中）及华东二区（已开放）四地域，满足用户特定应用场景需求。为获得更低的访问延迟，可选择在靠近您所在的地域及可用区部署。
- **多种实例规格：**
提供标准计算型、计算效能型Ⅰ、计算效能型Ⅱ和标准存储型四种实例规格，满足用户不同业务场景。更多实例规格正在筹备中。
- **灵活计费方式：**
支持包年包月的计费方式，用户可按需购买，节约成本。
- **丰富镜像资源：**
支持CentOS（7.1、7.2、7.5和6.6）和Ubuntu（16.04和14.04），以及多种常用应用软件的安装。
- **4种磁盘种类：**
SAS、SATA、NVME、SSD，4种硬盘合理搭配集成于云物理服务器，给用户提供高速、可靠的存储空间。
- **提供NO RAID、RAID0、RAID1和RAID10四种模式：**
系统盘若为单块，则提供RAID0系统盘若为两块，则提供RAID1模式。计算型的数据盘提供NO RAID、RAID0、RAID1和RAID10四种模式，存储型的数据盘为NO RAID模式。
- **2种IP地址：**
