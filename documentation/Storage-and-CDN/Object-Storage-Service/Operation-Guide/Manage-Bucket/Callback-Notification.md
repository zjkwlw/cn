...
* Topic：当指定时间发生时，OSS会向此topic发布消息，格式为NS:endpoint1,endpoint2,endpoint3（必须以"NS:"开头，多个地址用","分隔，最多配置5个）
* Event：触发通知的事件类型
...
```
### 回调签名
如果您的回调服务器被人恶意攻击了，例如恶意回调您的应用服务器，导致应用服务器收到一些非法的请求，影响正常逻辑，此时您就需要判断回调请求是否来自OSS。验证回调签名请参考文档：[回调签名认证]()。
