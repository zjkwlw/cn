...
|Region|地域或者数据中心|存储集群所处数据中心，目前有华北、华南、华东三个数据中心|
...
|Object Meta|文件元数据，用来描述文件信息，如长度，类型等|一组键值(Key-Value)的组合，分为系统预定义元数据(System-DefinedMetadata)和用户自定义元数据(User-Defined Metadata)两部分|
